...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  if (!sourcePolicyId || !targetProductId) {
    return res.status(400).json({
      message: "sourcePolicyId and targetProductId are required fields",
    })
  }

  // 1) Read source policy
  const src = await fetch(
    `https://api.keygen.sh/v1/accounts/${account}/policies/${sourcePolicyId}`,
    {
  ...
  if (!entitlementsToAttach) {
    const ents = await fetch(
      `https://api.keygen.sh/v1/accounts/${account}/policies/${sourcePolicyId}/entitlements`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
          Accept: "application/json",
  ...
  if (newId && entitlementsToAttach && entitlementsToAttach.length > 0) {
    await fetch(
      `https://api.keygen.sh/v1/accounts/${account}/policies/${newId}/relationships/entitlements`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
        ...
        body: JSON.stringify({
          data: entitlementsToAttach.map((eid) => ({
            type: "entitlements",
            id: eid,
          })),
        }),
