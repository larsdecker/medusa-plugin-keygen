...
describe("Policies routes", () => {
  ...
  it("creates a policy", async () => {
    ...
    global.fetch = vi.fn().mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        data: { id: "pol_new", attributes: { name: "Annual – 2 Seats" } },
      }),
    })

    const req: any = {
      body: { productId: "prod_1", name: "Annual – 2 Seats", maxMachines: 2 },
    }
  ...
  it("clones a policy", async () => {
    ;(global as any).fetch = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({
          data: { id: "pol_src", attributes: { name: "Src", maxMachines: 2 } },
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
        json: async () => ({ data: { id: "pol_cloned" } }),
      })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ data: [] }) })

    const req: any = {
      body: { sourcePolicyId: "pol_src", targetProductId: "prod_2" },
    }
    const res: any = mockRes()
